...
The prompt explains five types of verification hints the agent needs to add (invariants, assertions, pre/postconditions, termination measures), emphasizes the iterative workflow with the `verify_dafny` tool, and provides a concrete example[^2].

[^2]: Note the double braces `{{}}` in the example—these escape the braces so they're not interpreted as format string placeholders.
