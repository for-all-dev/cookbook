...
The implementation consists of four Python modules totaling ~400 lines of code:
- **`__init__.py`** (320 lines): Task definition, scorer, and system prompt
- **`dataset.py`** (32 lines): HuggingFace dataset loader
- **`tools.py`** (59 lines): The `verify_dafny` tool
- **`metrics.py`** (46 lines): Custom metrics for tracking verification performance
...
The main module brings everything together. Let's walk through it section by section.

### System Prompt

First, we define the system prompt that teaches the agent about Dafny verification:

```{literalinclude} ../../evals/src/evals/dafnybench/inspect_ai/__init__.py
:language: python
:caption: System prompt explaining Dafny verification
:linenos:
:start-at: # System prompt explaining Dafny verification
:end-at: """
:emphasize-lines: 15-19
```

The prompt explains five types of verification hints the agent needs to add (lines 46-50 in the original file), emphasizes the iterative workflow with the `verify_dafny` tool (lines 52-56), and provides a concrete example (lines 58-86).
...
The agent's completion might include markdown formatting or explanations. We extract just the Dafny code:

```{literalinclude} ../../evals/src/evals/dafnybench/inspect_ai/__init__.py
:language: python
:caption: Extract Dafny code from model completion
:linenos:
:start-at: def extract_code(completion: str) -> str:
:end-before: def categorize_error(stderr: str) -> str:
```

This regex finds code blocks marked with ` ```dafny ` or just ` ``` `, extracts the code, and returns the last match (since the model might explain its reasoning before providing the final code).

### Error Categorization

To understand failure modes, we categorize Dafny errors by type:

```{literalinclude} ../../evals/src/evals/dafnybench/inspect_ai/__init__.py
:language: python
:caption: Categorize Dafny verification errors
:linenos:
:start-at: def categorize_error(stderr: str) -> str:
:end-before: @scorer(
```

This pattern matching identifies common verification failures: invariant violations, assertion failures, postcondition issues, etc. The category is stored in score metadata and aggregated by the `error_type_distribution` metric.
...
The entire evaluation is ~400 lines of Python. Compare this to building from scratch with the Anthropic SDK—you'd need to implement:
