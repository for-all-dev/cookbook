...
```{literalinclude} ../../evals/src/evals/dafnybench/common/dataset.py
:language: python
:caption: evals/src/evals/dafnybench/common/dataset.py
:linenos:
```
...
`sandbox().exec` is Inspect's `subprocess.run` with extra steps. It's how you run shell commands from python.
...
One important thing to notice, which we'll discuss more in the next chapter, is use of the `raise` keyword. Here, we're using the notion of exception that means an error is _when things go as planned_, since we do not expect the agent to win on the first try, which may be offensive to some error handling purists. Inspect's tool call abstraction simply requires tools to be in an error state whenever the verification fails.
...
## Extracting code from the raw response

The utilities module provides a helper function for code extraction:
...
5. **Returns a Score** with:
   ...
   - `explanation`: Success message or error output
...
No manual loop management (`inspect_ai.solver.generate` is this for free!). Routing a toolcall from the model to the actual tool, as well as lots of logging facilities, come with Inspect out of the box. 

### CLI

There is further code in the CLI (serving this `dafnybench` function with `inspect_ai.eval` so we can `uv run` it), but it is not significant.
