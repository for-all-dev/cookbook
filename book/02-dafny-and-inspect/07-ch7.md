...
4. `inspect_ai`'s `.eval` file is none other than a zipped archive of `.json` files. Unzip it, poke around, and ship a `streamlit` dashboard that replaces `inspect_ai`'s generic `uv run inspect view` dashboard. This way, you can make it better suited for your needs. 

## Next steps

Instead of retrofitting the `inspect-ai` codebase for the hint-filling critique, we will move on. We will throw out our code up till this point and do it again _with no framework_. We'll use plain python with the [Anthropic SDK](https://pypi.org/project/anthropic/), to dispell any sense of "magic" behind the agentdev frameworks[^1] you'll use in the future. 
