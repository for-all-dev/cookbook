...
## TODO: audit the prose (so its not too llmy)

What `inspect-ai`'s `generate()` abstracts away is actually quite mechanical: a loop that calls the API, checks for tool use, executes tools, and feeds results back. Let's see what that looks like with plain Python and the Anthropic SDK.
